...
export ECS_TASK=$IMAGE_NAME-task_container
...
echo "Claster name: $AWS_ECS_CLUSTER_NAME"
...
if [ "$(aws ecs list-tasks --cluster $AWS_ECS_CLUSTER_NAME --service-name $ECS_SERVICE --region $AWS_DEFAULT_REGION | jq '.taskArns' | jq 'length')" -gt "0" ]; then
    TEMP_ARN=$(aws ecs list-tasks --cluster $AWS_ECS_CLUSTER_NAME --service-name $ECS_SERVICE --region $AWS_DEFAULT_REGION | jq '.taskArns[0]') # Get current running task ARN
    TASK_ARN="${TEMP_ARN%\"}" # strip double quotes
    TASK_ARN="${TASK_ARN#\"}" # strip double quotes
    aws ecs stop-task --cluster $AWS_ECS_CLUSTER_NAME  --task $TASK_ARN --region $AWS_DEFAULT_REGION > /dev/null # Stop current task to force start of new task revision with new image
fi
